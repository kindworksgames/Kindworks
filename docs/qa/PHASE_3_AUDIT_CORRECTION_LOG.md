...
| KW-XAUD-005 | Harbour General's recovered interior had become a complete environmental screenshot with live hit areas layered over it. | Replaced the production bitmap with a code-native full-shop composition derived from the approved layout: harbour windows, physical wall/island displays, side stock, customers, checkout and a selected-product management panel. The archived reference remains hash-pinned evidence only. | None. Ownership, stock, prices, six-slot assignment, four-item restocking, sales, wardrobes and till state are unchanged. | Live shelf swap/restock/reload at phone, tablet and desktop sizes; Harbour General/reference tests; exact archived-asset hash test; 606-test suite; production build/performance budget. | Corrected and browser verified. |
