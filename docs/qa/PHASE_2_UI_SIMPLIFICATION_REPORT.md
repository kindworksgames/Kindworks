...
| PlaygroundPowerwashScene | Wash the playground | Town playground/campaign | Player picker, 750-level/development copy, mask/tolerance/stroke result detail and a detached tool rail that did not match the HTML composition | CONVERT TO CONTEXTUAL UI | Start assigned/current playground immediately; make the approved 3:2 image the full interface; draw Soap/Precision/Standard/Wide into the image; brief reward | Complete |
...
| Playground Power Wash level picker | Town progression already determines the active playground and the legacy HTML used one image-led wash surface | Town interaction → 750-entry picker → detached dashboard → Start | Town interaction → approved playground image with integrated tool buttons | 580/580 tests; build/performance pass; exact nozzle/soap selection, touch wash, restart, reload, rotation, reward and Town return pass |
...
| Level, clean percentage, water, soap, projected reward, campaign totals and right-side tool rail | Playground Power Wash | Duplicated/debug-only and visually unfaithful | Removed from visible play; four protected tools are drawn into the approved artwork with aligned touch targets | Restores the HTML image-led composition while the exact engine state remains available to saves/tests |
...
| Beach Cleanup | `phase2-evidence/wave4-beach/beach-picker-568x320-after.png` | `phase2-ui-evidence/after/beach-direct-568x320.png` | 568×320; 390×844 rotate state | 1024×768 | Direct entry without picker, in-game hint plus movement changed rake progress 0/24→1/24 and rendered the protected grooves; orientation restored exact active beach | `eee3053` |
| Playground Power Wash | `docs/qa/evidence/phase2-ui/powerwash/before-active-568x320.png`: detached dashboard/right rail | `docs/qa/evidence/phase2-ui/powerwash/after-integrated-568x320.png`; `result-844x390.png`; `rotate-390x844.png` | All nine required landscape sizes; 390×844 rotate state | 1024×768, 1180×820 | Direct entry; Soap/Precision/Standard/Wide hit targets; real wash 0%→3%; reload retained 3%/Wide; Restart restored 0%/Precision; reward +100 and one Continue; exact rotation resume; 580/580 tests | `60863b7` |
...
| KW-UI-007 | Playground Power Wash | Restore the HTML image-led composition and remove its redundant player dashboard | `index.html`, protected Power Wash tool assets/manifest/extractor, `src/rendering/LegacyPowerwashRenderer.js`, `src/scenes/PlaygroundPowerwashScene.js`, `src/style.css` | Navigation and presentation only; active/current level starts automatically and tool hit targets align to the drawn HTML controls | No schema, level, reward, mask, tool, resource, town-restoration or checkpoint change | 16 focused tests plus 580-test full suite, build/performance and live viewport/input/reload/rotation/completion verification | Complete |
