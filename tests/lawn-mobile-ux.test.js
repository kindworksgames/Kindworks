...
test("keeps the Lawn Care board swipe-first with no visible direction pad", async () => {
  ...
  assert.doesNotMatch(styles, /\.lawn-care-controls|grid-template-areas: "undo up/);
  ...
  assert.match(styles, /\.lawn-care-side button \{ min-height: var\(--kw-touch-min\); \}/);
  assert.match(styles, /\.lawn-board-viewport \{ min-height: 0; padding: 3px; \}/);
