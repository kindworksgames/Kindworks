...
function validFrame(level, frame) {
  ...
  return {
    ...
    cutCells: cellsFromMask(level, maskFromCells(level, frame.cutCells)),
...
export class LawnCareEngine {
  ...
  move(directionValue) {
    ...
    this.moves += 1;
    this.won = this.cutMask === this.level.allCutMask;
    this.ended = this.won || this.moves >= this.moveLimit;
    const newlyCut = cellsFromMask(this.level, this.cutMask & ~beforeMask);
    return {
      ok: true,
      code: this.won ? "lawn-perfect" : this.ended ? "lawn-ended" : "lawn-moved",
      direction,
