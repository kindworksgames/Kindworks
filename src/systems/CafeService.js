...
export class CafeService {
  ...
  subscribe(listener) {
    if (typeof listener !== "function") throw new TypeError("A café listener must be a function.");
  ...
  getSnapshot() { return structuredClone(this.gameState.getSnapshot().cafe); }
  ...
  commit(mutator) {
    ...
    if (!saved.ok) {
      ...
      return { ok: false, code: "persistence-failed", message: "The café result could not be saved, so progress and coins were restored safely.", save: saved, rollbackOk: rollback.ok };
    }
  ...
  startLevel(number = 1, { returnPosition, returnFacing = "down", instantOrders = false } = {}) {
    if (this.activeSession && !this.activeSession.finished) return { ok: false, code: "shift-active", message: "Finish or safely exit the current café shift first." };
    const level = cafeLevel(number);
    const progress = this.getSnapshot();
    if (level.level > progress.unlockedLevel) return { ok: false, code: "level-locked", message: "Complete the previous café shift first." };
    const names = NPC_RESIDENTS.map((resident) => resident.name);
    const orders = level.orders.map((order, index) => ({
      id: `cafe-order-${index + 1}`,
    ...
    this.activeSession = {
      id: `cafe-shift-${String(this.nextSessionId).padStart(4, "0")}`,
      level,
      orders,
      spawnIndex: 0,
      activeOrderIds: [],
    ...
    this.nextSessionId += 1;
    this.spawnOrders();
    return { ok: true, code: "cafe-shift-started", session: this.getActiveSession(), expectedStep: this.expectedStep() };
  }
  ...
  spawnOrders() {
    ...
    let spawned = 0;
    while (session.spawnIndex < session.orders.length && session.orders[session.spawnIndex].at <= session.elapsed && session.activeOrderIds.length < CAFE_CONFIG.maxCustomers) {
  ...
  recordMistake(message, tray = this.tray()) {
    const session = this.activeSession;
    if (!session || session.finished) return { ok: false, code: "no-active-shift", message: "Start a café shift first." };
    session.mistakes += 1;
    const order = this.orderForTray(tray);
    if (order) order.patience = Math.max(-CAFE_CONFIG.graceSeconds, order.patience - 1.5);
  ...
  applyStep(stepId, trayIndex = this.activeSession?.activeTray) {
    ...
    const result = applyRecipeStep(tray, recipe, stepId, cafeStep);
    if (!result.ok && result.code === "unknown-step") return this.recordMistake("Choose a valid café ingredient or kitchen station.", tray);
    if (!result.ok && result.code === "wrong-step") return this.recordMistake(`That dish needs ${cafeStep(result.expectedStep).name} next.`, tray);
  ...
  serveActive(trayIndex = this.activeSession?.activeTray) {
    ...
    session.activeTray = tray.index;
    if (tray.stepIndex !== recipe.steps.length) return this.recordMistake(`${recipe.name} still needs ${cafeStep(this.expectedStep(tray)).name}.`, tray);
    ...
    if (session.served >= session.level.target && session.spawnIndex >= session.orders.length && session.activeOrderIds.length === 0) {
      ...
      if (finished.ok) return { ...finished, code: "cafe-shift-complete", customerName: order.customerName, customerOutcome: "loved" };
  ...
  finishSession({ failureReason = null } = {}) {
    const session = this.activeSession;
    if (!session || session.finished) return { ok: false, code: "shift-already-finished", message: "This café shift is already finished." };
    ...
    const transaction = this.commit((state) => {
      ...
      if (result.won) {
        ...
        progress.unlockedLevel = Math.max(progress.unlockedLevel, Math.min(CAFE_CONFIG.levelCount, level + 1));
        if (firstClear) {
          coins = cafeFirstClearCoins(level, result.stars);
          ...
          ledger = appendLedger(state, this.now(), { amount: coins, kind: "corner-cafe-first-clear", reason: `Corner Café Level ${level} first clear`, level, score: result.score, stars: result.stars, served: result.served, venue: "corner" });
        }
      }
      progress.shifts += 1;
      ...
      return { ok: true, code: result.won ? "cafe-result-won" : "cafe-result-lost", result: { ...result, coins, firstClear }, coins, firstClear, ledger };
    });
  ...
  cancel() {
    if (!this.activeSession) return { ok: false, code: "no-active-shift", message: "No café shift is open." };
    const session = this.getActiveSession();
    this.activeSession = null;
    return { ok: true, code: "cafe-shift-cancelled", session };
  }

  getDiagnostics() {
    const state = this.getSnapshot();
    return {
      levels: CAFE_LEVELS.length,
      recipes: Object.keys(CAFE_RECIPES).length,
      ingredients: Object.keys(CAFE_INGREDIENTS).length,
      appliances: Object.keys(CAFE_APPLIANCES).length,
      trays: CAFE_CONFIG.trayCount,
