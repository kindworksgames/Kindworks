...
import { normalizeMorningMugActiveShift } from "../state/morningMugState.js";
import {
  applyRecipeStep,
  recipeForOrder,
  recipeOrderScore,
...
export class MorningMugService {
  constructor(gameState, repository, { now = () => Date.now() } = {}) {
    this.gameState = gameState;
    this.repository = repository;
    this.now = now;
    this.activeSession = normalizeMorningMugActiveShift(this.getSnapshot().activeShift);
  ...
  subscribe(listener) {
    if (typeof listener !== "function") throw new TypeError("A Morning Mug listener must be a function.");
  ...
  getSnapshot() { return structuredClone(this.gameState.getSnapshot().morningMug); }
  ...
  commit(mutator) {
    ...
    if (!saved.ok) {
      ...
      return { ok: false, code: "persistence-failed", message: "The Morning Mug save failed, so the shift, progress and coins were restored safely.", save: saved, rollbackOk: rollback.ok };
    }
  ...
  persistActiveSession() {
    const activeShift = this.activeSession && !this.activeSession.finished
      ? structuredClone(this.activeSession)
      : null;
    const persisted = this.commit((state) => {
      state.morningMug.activeShift = activeShift;
      return { ok: true, code: activeShift ? "morning-mug-shift-checkpointed" : "morning-mug-shift-cleared" };
    });
  ...
  restorePersistedSession() {
    if (this.activeSession && !this.activeSession.finished) return { ok: true, code: "morning-mug-shift-already-active", session: this.getActiveSession(), expectedStep: this.expectedStep() };
    const restored = normalizeMorningMugActiveShift(this.getSnapshot().activeShift);
    if (!restored) return { ok: false, code: "no-saved-shift", message: "No Morning Mug shift is waiting to resume." };
    this.activeSession = restored;
    this.lastCheckpointSecond = Math.floor(restored.elapsed);
    return { ok: true, code: "morning-mug-shift-resumed", session: this.getActiveSession(), expectedStep: this.expectedStep() };
  }

  startLevel(number = 1, { returnPosition, returnFacing = "down", instantOrders = false } = {}) {
    if (this.activeSession && !this.activeSession.finished) return { ok: false, code: "shift-active", message: "Resume or save and exit the current Morning Mug shift first." };
    if (this.getSnapshot().activeShift) return { ok: false, code: "resume-available", message: "A saved Morning Mug shift is ready to resume." };
    const level = morningMugLevel(number);
    const progress = this.getSnapshot();
    if (level.level > progress.unlockedLevel) return { ok: false, code: "level-locked", message: "Complete the previous Morning Mug shift first." };
    ...
    const orders = level.orders.map((customerOrder, index) => ({
      ...
      customerName: names[(level.level * 2 + (index + 1) * 5) % names.length] || `Customer ${index + 1}`,
    ...
    this.activeSession = {
      id: `morning-mug-shift-${String(this.nextSessionId).padStart(4, "0")}`,
      level,
      orders,
      spawnIndex: 0,
      activeOrderIds: [],
      trays: Array.from({ length: MORNING_MUG_CONFIG.trayCount }, (_, index) => emptyTray(index)),
      appliances: createRestaurantAppliances(MORNING_MUG_APPLIANCES, null, MORNING_MUG_CONFIG.trayCount),
      activeTray: 0,
      elapsed: 0,
      served: 0,
      missed: 0,
    ...
    return { ok: true, code: "morning-mug-shift-started", session: this.getActiveSession(), expectedStep: this.expectedStep(), save: persisted.save };
  }
  ...
  spawnOrders() {
    ...
    let spawned = 0;
    while (session.spawnIndex < session.orders.length && session.orders[session.spawnIndex].at <= session.elapsed && session.activeOrderIds.length < MORNING_MUG_CONFIG.maxCustomers) {
  ...
  currentRecipe(tray = this.tray()) { return recipeForOrder(this.orderForTray(tray), tray?.recipeIndex || 0, MORNING_MUG_RECIPES); }
  ...
  useAppliance(stepId, trayIndex = this.activeSession?.activeTray, { durationScale = 1 } = {}) {
    ...
    const definition = MORNING_MUG_APPLIANCES[stepId];
    ...
    if (!definition || !appliance) return this.recordMistake("Choose a valid Morning Mug station.", tray);
    if (appliance.status === "burnt") {
      clearRestaurantAppliance(appliance);
      session.waste += 1;
      session.mistakes += 1;
      return this.persistMutation(checkpoint, { ok: false, code: "station-burnt", message: `${definition.name} burnt. Cleared—make that step again.`, waste: session.waste, mistakes: session.mistakes });
    }
    if (appliance.status === "cooking") return { ok: false, code: "station-cooking", message: `${definition.name} is still working.` };
    if (appliance.status === "ready") {
      ...
      const result = applyRecipeStep(owner, recipe, stepId, morningMugStep);
    ...
    if (expected !== stepId) { this.activeSession = checkpoint; return this.recordMistake(expected ? `That drink needs ${morningMugStep(expected).name} next.` : "Finish the prepared drink first.", this.tray(trayIndex)); }
    if (!startRestaurantAppliance(session, MORNING_MUG_APPLIANCES, stepId, tray.index, durationScale)) return { ok: false, code: "station-unavailable", message: `${definition.name} is unavailable.` };
  ...
  selectTray(index) {
    ...
    if (!session || session.finished || !tray?.orderId) return { ok: false, code: "tray-unavailable", message: "Choose an occupied drink preparation tray." };
    const checkpoint = structuredClone(session);
  ...
  recordMistake(message, tray = this.tray()) {
    const session = this.activeSession;
    if (!session || session.finished) return { ok: false, code: "no-active-shift", message: "Start a Morning Mug shift first." };
    const checkpoint = structuredClone(session);
    session.mistakes += 1;
    const customerOrder = this.orderForTray(tray);
    if (customerOrder) customerOrder.patience = Math.max(-MORNING_MUG_CONFIG.graceSeconds, customerOrder.patience - 1.5);
    const saved = this.persistActiveSession();
    if (!saved.ok) {
      this.activeSession = checkpoint;
      return saved;
  ...
  applyStep(stepId, trayIndex = this.activeSession?.activeTray) {
    ...
    const result = applyRecipeStep(tray, recipe, stepId, morningMugStep);
    if (!result.ok && result.code === "unknown-step") { this.activeSession = checkpoint; return this.recordMistake("Choose a valid Morning Mug ingredient or station.", this.tray(trayIndex)); }
    if (!result.ok && result.code === "wrong-step") { this.activeSession = checkpoint; return this.recordMistake(`That drink needs ${morningMugStep(result.expectedStep).name} next.`, this.tray(trayIndex)); }
  ...
  serveActive(trayIndex = this.activeSession?.activeTray) {
    ...
    const customerOrder = this.orderForTray(tray);
    const recipe = this.currentRecipe(tray);
    if (!session || session.finished || !tray || !customerOrder || !recipe) return { ok: false, code: "no-active-preparation", message: "Choose an active customer tray first." };
    session.activeTray = tray.index;
    if (tray.stepIndex !== recipe.steps.length) return this.recordMistake(`${recipe.name} still needs ${morningMugStep(this.expectedStep(tray)).name}.`, tray);
    const checkpoint = structuredClone(session);
    tray.completedRecipes.push(customerOrder.recipes[tray.recipeIndex]);
    tray.recipeIndex += 1;
    resetRecipePreparation(tray);
    if (tray.recipeIndex < customerOrder.recipes.length) {
      return this.persistMutation(checkpoint, { ok: true, code: "drink-added", message: `${recipe.name} added to the order.`, nextRecipe: this.currentRecipe(tray), tray: structuredClone(tray) });
    }
    ...
    if (session.served >= session.level.target && session.spawnIndex >= session.orders.length && session.activeOrderIds.length === 0) {
      ...
      if (finished.ok) return { ...finished, code: "morning-mug-shift-complete", customerName: customerOrder.customerName, customerOutcome: "loved" };
      this.activeSession = checkpoint;
      return finished;
    }
    return this.persistMutation(checkpoint, { ok: true, code: "customer-served", customerName: customerOrder.customerName, customerOutcome: "loved", served: session.served, nextCustomer: this.orderForTray(this.tray())?.customerName || null, session: this.getActiveSession() });
  }
  ...
  tick(seconds) {
    ...
    for (const customerOrder of this.activeOrders()) {
      ...
      if (customerOrder.patience <= -MORNING_MUG_CONFIG.graceSeconds) {
        session.missed += 1;
        session.streak = 0;
        session.failureReason = `${customerOrder.customerName} left before the drink was ready.`;
    ...
    if (session.elapsed >= session.level.duration) {
      ...
      session.streak = 0;
      session.failureReason = "The Morning Mug shift timer ended.";
      const finished = this.finishSession({ failureReason: session.failureReason });
      if (!finished.ok) this.activeSession = checkpoint;
      return finished;
    }
    ...
    return { ok: true, code: "morning-mug-tick", remaining: session.level.duration - session.elapsed, spawned, activeOrders: session.activeOrderIds.length, applianceChanges };
  }

  finishSession({ failureReason = null } = {}) {
    const session = this.activeSession;
    if (!session || session.finished) return { ok: false, code: "shift-already-finished", message: "This Morning Mug shift is already finished." };
    ...
    const transaction = this.commit((state) => {
      const progress = state.morningMug;
      ...
      if (result.won) {
        ...
        progress.unlockedLevel = Math.max(progress.unlockedLevel, Math.min(MORNING_MUG_CONFIG.levelCount, level + 1));
        if (firstClear) {
          coins = morningMugFirstClearCoins(level, result.stars);
          ...
          ledger = appendLedger(state, this.now(), { amount: coins, kind: "morning-mug-first-clear", reason: `Morning Mug Level ${level} first clear`, level, score: result.score, stars: result.stars, served: result.served, venue: "mug" });
        }
      }
      progress.shifts += 1;
      ...
      return { ok: true, code: result.won ? "morning-mug-result-won" : "morning-mug-result-lost", result: { ...result, coins, firstClear }, coins, firstClear, ledger };
    });
  ...
  suspend() {
    if (!this.activeSession || this.activeSession.finished) return { ok: false, code: "no-active-shift", message: "No active Morning Mug shift needs saving." };
    const session = this.getActiveSession();
    const persisted = this.persistActiveSession();
    if (!persisted.ok) return persisted;
    this.activeSession = null;
    return { ok: true, code: "morning-mug-shift-suspended", session, save: persisted.save };
  }

  cancel() {
    if (!this.activeSession && !this.getSnapshot().activeShift) return { ok: false, code: "no-active-shift", message: "No Morning Mug shift is open." };
    const session = this.getActiveSession() || normalizeMorningMugActiveShift(this.getSnapshot().activeShift);
    ...
    return { ok: true, code: "morning-mug-shift-cancelled", session, save: cleared.save };
  }

  getDiagnostics() {
    const state = this.getSnapshot();
    const active = this.activeSession || normalizeMorningMugActiveShift(state.activeShift);
    return {
      levels: MORNING_MUG_LEVELS.length,
      recipes: Object.keys(MORNING_MUG_RECIPES).length,
      ingredients: Object.keys(MORNING_MUG_INGREDIENTS).length,
      appliances: Object.keys(MORNING_MUG_APPLIANCES).length,
      trays: MORNING_MUG_CONFIG.trayCount,
